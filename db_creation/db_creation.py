...
def drop_tables(conn: sqlite3.Connection):
    """ !!!CAREFUL!!! FULL DELETION. Call ONLY to start the db from scratch """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--csv-data-path", help="path to the structured .csv input file with prefounded questions and answers",
        default='../data/interview_answers.csv'
    )
    parser.add_argument("--new-csv-data-path", help="path to the reindex .csv",
        default='../data/interview_answers_reindex.csv'
    )
    args = parser.parse_args()

    interview_data = pd.read_csv(args.csv_data_path)
    ...
    answers.to_csv(args.new_csv_data_path, index=False)
    # will create the db if not exist.
